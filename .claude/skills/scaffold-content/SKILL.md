...
These apply to every stub generated by this skill and to any loader code written alongside it.

- **All required fields must be present in the YAML.** Do not rely on silent code-side defaults. A stub that ships with a required field missing is a bug, not a convenience.
- **Never generate loader code that does `.get("key", <fallback>)` for required fields.** Use `data["key"]` (or a helper that raises). Hardcoded defaults in Python mask missing YAML data and drift from design docs.
- **Never add method parameter defaults (`arg="still"`, `qty=1`, etc.) for values that the YAML is supposed to carry.** Require them; let the caller or YAML supply them.
- **When a required value is missing or `None`, raise `ValueError` with file, property path, and example.** Template:

  ```python
  if entry.get("chase_range") is None:
      raise ValueError(
          f"Encounter zone {zone_id!r} entry is missing 'chase_range'. "
          f"Define it in rusted_kingdoms/data/encount/{zone_id}.yaml under entries[*].chase_range "
          f"(e.g., 3 for a 3-tile chase radius; use 0 for non-chasing)."
      )
  ```

- **If the stub has a placeholder like `<int>` or `""`, call it out.** Tell the user that value is required and the loader will raise on load if it stays blank.

When generating a stub, strip any field that previously only existed as a silent fallback in Python and instead surface it in the stub (with a clear placeholder) so the YAML is the single source of truth.
