...
def main() -> None:
    ...
    parser.add_argument("input", type=Path, help="Input TSX file.")
    ...
    wangsets = choose_wangsets(root, args.wangset)
    graphs = [graph_for_wangset(wangset) for wangset in wangsets]

    default_base = args.input.with_suffix("")
    ...
    if args.wangset and len(graphs) == 1:
