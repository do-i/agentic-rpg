...
from engine.common.ui.theme import EMBER, GOLD
from engine.common.ui.chrome import render_modal, render_panel, wrap_text
...
class MagicCoreShopRenderer:
    ...
    def __init__(self) -> None:
        self._fonts = FontSet(
            title=(24, True), row=18, gp=18, qty=(22, True),
            arrow=22, hint=15, toast=(22, True), confirm=17,
        )
    ...
    def _draw_qty_overlay(
        ...
        lbl = self._fonts.row.render(label, True, C_HEADER)
        ...
        max_s = self._fonts.hint.render(f"max {max_qty}", True, C_DIM)
        screen.blit(max_s, (ox + ow - max_s.get_width() - 16, oy + 52))

        total_s = self._fonts.gp.render(f"   {total:,} GP", True, C_GP_GAIN)
    ...
    def _draw_confirm_overlay(
        ...
        lines = wrap_text(self._fonts.confirm, msg, inner)
        line_h = self._fonts.confirm.get_height() + 4
        hint = self._fonts.hint.render(
        ...
        for line in lines:
            screen.blit(
                self._fonts.confirm.render(line, True, C_CONFIRM_TXT),
                (modal.x + 20, y))
            y += line_h
    ...
    def _draw_popup(self, screen: pygame.Surface, popup_text: str) -> None:
        pw = 460
        lines = wrap_text(self._fonts.toast, popup_text, pw - 40)
        line_h = self._fonts.toast.get_height() + 4
        ph = 18 + len(lines) * line_h + 32
        modal = render_modal(screen, pw, ph)
        y = modal.y + 18
        for line in lines:
            msg = self._fonts.toast.render(line, True, C_GP_GAIN)
            screen.blit(msg, (modal.x + (pw - msg.get_width()) // 2, y))
            y += line_h
        hint = self._fonts.hint.render("ENTER / ESC  close", True, C_HINT)
